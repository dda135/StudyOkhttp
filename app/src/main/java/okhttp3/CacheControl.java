  ...
  public static CacheControl parse(Headers headers) {
    ...
    for (int i = 0, size = headers.size(); i < size; i++) {
      ...
      if (name.equalsIgnoreCase("Cache-Control")) {
        ...
        //在Http1.1的协议当中，Pragma:no-cache等价于cache-control:no-cache
        //所以和cache-control为互斥概念
        canUseHeaderValue = false;
      } else {//只解析头部报文中Cache-Control和Pragma
        continue;
      }
      ...
      while (pos < value.length()) {
        ...
        //获得Cache-Control解析后的value，该报文结构对应request和response有所不同
        if ("no-cache".equalsIgnoreCase(directive)) {//request/response报文，指示不能缓存
          noCache = true;
        } else if ("no-store".equalsIgnoreCase(directive)) {//request/response报文，指示不能使用缓存（不代表不能进行缓存）
          noStore = true;
        } else if ("max-age".equalsIgnoreCase(directive)) {//当前
          maxAgeSeconds = HttpHeaders.parseSeconds(parameter, -1);
        } else if ("s-maxage".equalsIgnoreCase(directive)) {
          sMaxAgeSeconds = HttpHeaders.parseSeconds(parameter, -1);
        } else if ("private".equalsIgnoreCase(directive)) {//response报文，指示当前response非共享缓存，
          isPrivate = true;
        } else if ("public".equalsIgnoreCase(directive)) {//response报文，指示当前response可以以任何形式进行缓存
          isPublic = true;
