 ...
 * responses.
 * 工厂模式，用于发送HTTP请求和读取对应的相应结果
 *
 * <h3>OkHttpClients should be shared</h3>
 * 工厂单例应该被分享，简单说就是整个项目应该只维护一个OkHttpClients
 *
 ...
 * 单例模式的原因：
 * 每一个OkHttpClient都持有自己的连接池和线程池，重用连接和线程可以减少和节省内存。
 *
 ...
 * 使用Builder模式主要可以配置一些基础参数，用来处理一些特殊的情况
 *
 ...
 * }</pre>
 * 如果Shutdown分发者的线程池，会导致线程池永久的关闭（线程池一旦关闭不可重启）
 * 这样后续的请求都会被拒绝
 *
 ...
 * }</pre>
 * 可以通过evictAll来清理连接池。注意连接池的守护线程不会立刻退出
 *
 ...
 *     client.cache().close();
 * }</pre>
 * 如果你的client有缓存，你可以通过close来关闭缓存的使用。
 * 注意close之前应该检查缓存是否已经关闭，否则可能会抛出异常
 *
 * <p>OkHttp also uses daemon threads for HTTP/2 connections. These will exit automatically if they
 * remain idle.
 * OkHttp在Http2协议的连接中使用的是守护线程，这样可以在空闲的时候自动退出
 */
public class OkHttpClient implements Cloneable, Call.Factory, WebSocket.Factory {
  ...
  public static final class Builder {
    ...
    //connection创建完成之后执行，注意此时并没有向服务端发送数据
    final List<Interceptor> networkInterceptors = new ArrayList<>();
    //代理选择器，简单理解就是针对不同的域名可以使用不同的代理
    ProxySelector proxySelector;
    //cookie的管理器，可以实现cookie的存储等情况
    CookieJar cookieJar;
    //缓存，用于处理cache-control的情况
    Cache cache;
    //如果指定cache，则internalCache默认为cache，一般没必要指定这个
    InternalCache internalCache;
    //socket套接字工厂
    ...
    //连接池，内部维护一些RealConnection，主要考虑到相同域名等情况下的Connection重用
    ConnectionPool connectionPool;
    //自定义的域名到IP的转换
    Dns dns;
    //在允许重定向的基础上，是否允许SSL重定向，在第一次回话完成之后，可能会返回重定向
    boolean followSslRedirects;
    //是否允许重定向，当返回code为300多的时候来决定是否再次发出请求
    boolean followRedirects;
    //当发出一次请求出现异常的时候是否需要尝试重连
    boolean retryOnConnectionFailure;
    //连接超时时长
    int connectTimeout;
    //source.timeout().timeout(readTimeout, MILLISECONDS);
    //sink.timeout().timeout(writeTimeout, MILLISECONDS);
    //可以看到实际上对应的是往服务端写的时间和读取流的时间
    int readTimeout;
    int writeTimeout;
    int pingInterval;
    ...
    public Builder protocols(List<Protocol> protocols) {
      ...
      if (protocols.contains(Protocol.SPDY_3)) {//注意到SPDY_3协议已经被放弃，目前主要是采用http2
