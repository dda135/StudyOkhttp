...
public final class Response implements Closeable {
   ...
   * 在OKHttp传输已经初始化之后的request到网络上之前记录的时间戳
   * 如果response从缓存中获得（说明之前request已经从网络上获取过数据），则该时间对应的是最早的发送的网络请求的时间戳
   */
   ...
   * 返回在OkHttp从网络上接收完的response的头部报文的时间戳
   * 如果这个response从缓存中获得，则该时间戳为第一次从网络上获取后的response的时间戳
   */
